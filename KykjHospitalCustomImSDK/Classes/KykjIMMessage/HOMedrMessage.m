...
@implementation HOMedrMessage
///初始化
+ (instancetype)messageWithContent:(NSString *)content {
    HOMedrMessage *text = [[HOMedrMessage alloc] init];
    if (text) {
...
- (void)decodeWithData:(NSData *)data {
    if (data) {
        ...
        if (dictionary) {
            ...
            NSDictionary * extraDict = [self.extra mj_JSONObject];
            self.extraModel = [HOIMPreMsgExtraModel mj_objectWithKeyValues:extraDict];
...
+ (NSString *)getObjectName {
    return HOMedrMessageTypeIdentifier;
}
@end
