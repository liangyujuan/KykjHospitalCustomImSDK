    ...
    case CallAction_Call:
        {
            ...
            if (isGroup) {
                ...
                callID = [[V2TIMManager sharedInstance] invite:realModel.invitedList.firstObject data:data onlineUserOnly:YES offlinePushInfo:info timeout:SIGNALING_EXTRA_KEY_TIME_OUT succ:^{
                    @strongify(self)
                    // 发起 Apns 推送
...
//                }];
                ...
                if (self.startCallTS > 0) {
                    ...
                    callID = [[V2TIMManager sharedInstance] invite:receiver data:data onlineUserOnly:YES offlinePushInfo:info timeout:0 succ:nil fail:^(int code, NSString *desc) {
                        if ([self canDelegateRespondMethod:@selector(onError:msg:)]) {
                            [self.delegate onError:code msg:desc];
                        };
                    }];
